...
function connect(index, options) {
  const hosts = options.host.split(',');
  const url = 'couchbase://' + hosts[index] + ':' + options.port;
  if (index < hosts.length) {
    try {
      const cluster = new couchbase.Cluster(url);
      cluster.authenticate(options.rbacUsername, options.rbacPassword);
      cluster.openBucket(options.bucket);
      const bucket = cluster.openBucket(options.bucket);
      bucket.connectionTimeout = 20000;
      return {cluster, bucket};
    } catch (e) {
      console.log('error', e)
      return connect(index++, hosts, port);
    }
  }
  return null;
}
...
Couchbase.prototype.connect = function(callback) {
  ...
  if (self.bucket) {
    ...
    const {cluster, bucket} = connect(0, options);
